...
nonisolated final class HistoryDatabase {
    ...
    /// 将旧 Flofoil/Flamina 支持目录迁移为 foofoil，仅当新目录不存在时执行。
    private static func migrateLegacySupportDirectoryIfNeeded() {
        ...
        let target = appSupportURL.appendingPathComponent("foofoil", isDirectory: true)
        guard !FileManager.default.fileExists(atPath: target.path) else { return }
        for legacyName in ["Flofoil", "Flamina"] {
            let legacy = appSupportURL.appendingPathComponent(legacyName, isDirectory: true)
    ...
    init(databaseURL: URL? = nil) throws {
        if let databaseURL {
            ...
            // 若迁移未执行但旧库仍存在（如新目录已存在导致未自动迁移），尝试复制旧数据库（兼容 Flofoil/Flamina）
            if !FileManager.default.fileExists(atPath: self.databaseURL.path) {
                for legacyName in ["Flofoil", "Flamina"] {
                    let legacyDB = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
                    ...
                    if FileManager.default.fileExists(atPath: legacyDB.path) {
                        try? FileManager.default.copyItem(at: legacyDB, to: self.databaseURL)
                        // 同时复制 WAL/SHM
                        for suffix in ["-wal", "-shm"] {
                            let legacyAux = URL(fileURLWithPath: legacyDB.path + suffix)
                            let targetAux = URL(fileURLWithPath: self.databaseURL.path + suffix)
                            if FileManager.default.fileExists(atPath: legacyAux.path) {
                                try? FileManager.default.copyItem(at: legacyAux, to: targetAux)
                            }
                        }
                        break
                    }
    ...
    private func openAndMigrate() throws {
        ...
        try execute("""
            CREATE TABLE IF NOT EXISTS history_items (
                ...
                media_sidecar_bookmark TEXT, custom_cover_path TEXT,
    ...
    private func displayTitle(for config: WindowConfig, kind: HistoryContentKind) -> String {
        // 列表标题与历史菜单保持同一来源；无标题时随数量变化重算回退标题。
        ...
        try withStatement(sql, bindings: bindings) { statement in
            ...
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                result.append(WindowConfig(
                    ...
                    customCoverPath: columns["custom_cover_path"].flatMap { optionalText(statement, $0) },
