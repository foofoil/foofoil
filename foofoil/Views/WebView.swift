...
struct WebView: NSViewRepresentable {
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler, WKUIDelegate {
        static let editableFocusHandlerName = "foofoilEditableFocus"
        static let editableFocusTrackingScript = """
        (() => {
            const handler = window.webkit?.messageHandlers?.foofoilEditableFocus;
            if (!handler) return;
        ...
        func setupObservers(webView: WKWebView) {
            ...
            // 监听特定窗口的截取图片箔通知
            let captureNotificationName = Notification.Name("captureImageFoofoil_\(parent.appState.id.uuidString)")
            captureNotificationToken = NotificationCenter.default.addObserver(
                ...
                webView.takeSnapshot(with: config) { nsImage, error in
                    if let image = nsImage {
                        ...
                        DispatchQueue.main.async {
                            self.parent.appState.createNewFoofoilFromScreenshot(image: image, backingScaleFactor: scale)
                        }
