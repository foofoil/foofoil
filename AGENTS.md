...
Host extension code lives in `foofoil/ExtensionSupport/`: `Runtime` owns providers, loading, sessions and optional device services; `Management` owns installation, compatibility negotiation and Registry; `Presentation` owns host UI, playback adapters and host queue projection; `Compatibility` holds legacy Hi-Fi fallbacks pending removal per the final review and must not gain new fallbacks. Keep playback controllers separate from views. `foofoil/Extensions/` remains Swift/system type extensions, not plugins. Xcode uses a filesystem-synchronized source group, so moves within the source root need no per-file project entries.

Prefer negotiated capabilities and opaque IDs. Never add Hi-Fi identifiers or command parsing to generic host paths. There are no historical users: the P0 support window is closed, so do not retain legacy session/media/navigation/restore or content-probe fallbacks for unreleased P0 protocols, and do not support old-host/new-extension or new-host/old-extension combinations; local development data may be cleared when needed. Device services have no fixed Hi-Fi ID fallback. See `docs/extension-boundary-refactor-review-final.zh-CN.md` for the current cleanup plan, ownership and verification.
...
- There are no historical users: persisted models such as `WindowConfig` do not require cross-version backward compatibility, and local development data may be cleared when needed. Within the current version, restart recovery and session rebuild must still work; use sensible defaults when adding fields.
