...
日期：2026-09-10  
状态：收尾阶段 0–6 全部完成；全部实机/界面手动项已由用户复验通过，最终验收通过  
...
- [x] 验证旧协议明确被拒绝、当前公共协议成功，菜单/媒体键/设备菜单仍可用，普通 PCM 无 hifi 时可播放。
- [x] 更新 smoke 的真实公共 fixture 参数、AGENTS/计划/支持文档，清除过时的继续兼容要求。
- [x] **验收：** 菜单、设备状态与协议迁移一并完成，无新路径暗中依赖兼容层；三仓库相关测试、ABI smoke 与 `./run` 通过。
...
- [x] 实机回归 PCM 系统/独占、同 DAC PCM/DSD 双向快速交接、DSF/DFF/SACD 播放/暂停/定位/切曲。
- [x] 实机回归删除当前曲、删除/裁剪/重排后继后的续播，以及容器激活/呈现。
- [x] 实机回归设备拔出/重接/忙碌/切换、关窗/退出释放、菜单与媒体键。
- [x] 实机回归 PCM 独立文件同采样率连播、CUE 首次播放；自动覆盖 44.1 kHz 单/双声道，防止仅单声道测试掩盖默认格式路径。
- [x] 汇总 foofoil/hifi/extension-kit 提交与工作区、设备型号或 UID、文件类型/采样率、操作与结果来源；不外推未测硬件。
- [x] 按职责审查残留标识：通用宿主不含 Hi-Fi 特例/适配调用；允许配置、文案、测试及插件自身的不透明标识，不以全局字符串零命中代替审查。
- [x] **最终验收：** 评审 §9 全部满足，所有必需手动验证通过；文档与实际状态一致，无未记录阻塞。
...
- 实机设备 / 文件类型与采样率 / 操作 / 结果来源：**用户实机复验通过**（结果来源为用户；设备型号/UID 与文件/采样率由用户在测试中指出，未在本文逐项转录）。扩展菜单不再出现旧命令、媒体键、设备菜单（含不可用设备禁用与失败不暂停）及无 hifi 普通 PCM 均通过。**4.2 设备菜单**的回归已修复：旧 `hifi.device.*` 的“当前速率兼容”语义迁到公共 `isCompatible`，宿主据此禁用；设备切换不再预先暂停，由扩展在 `selectDevice` 内校验并按需暂停/恢复。
...
- 验收是否通过及证据：通过。代码迁移、三仓库测试、ABI smoke、`./run` 及各实机项（用户复验）均通过。
- 下一阶段：收尾阶段 5（扫描、通用容器与余项）。

## 收尾阶段 5 执行记录（2026-09-11）— 已验收
...
- 构建、ABI smoke、`./run`：三仓库构建通过，`./run` `BUILD SUCCEEDED`、插件注入并启动，无新增编译警告。
- 实机设备 / 文件类型与采样率 / 操作 / 结果来源：**用户实机复验通过**（大列表连续扫描不阻塞、容器边界与 sniff 歧义、Provider 竞争/偏好、授权资源与书签、扫描取消/快速切换、通用容器呈现与激活）；性能结论仍以单元测试的调用计数与主线程证据为主。
- 未验证范围 / 风险 / 阻塞：一次性打开/恢复时的 `resolve/match` 探针仍在调用 actor 上（有资源作用域），未移出主 actor；作为已盘点边界保留，不影响本轮验收。
- 验收是否通过及证据：通过。扫描/授权/取消与通用容器测试、三仓库构建与 `./run`、以及用户实机复验均通过；未增加未经证实需要的缓存或并发基础设施。
- 下一阶段：收尾阶段 6（最终集成与交付）。

## 收尾阶段 6 执行记录（2026-09-11）— 已验收
...
- PCM 同采样率连播/CUE 首次播放：排查期间加入的临时诊断日志已移除、未保留噪声；用户复验通过。
- 实机设备 / 文件类型与采样率 / 操作 / 结果来源：**用户实机复验通过**（结果来源为用户；设备型号/UID、文件类型与采样率由用户在测试中指出，未在本文逐项转录）。覆盖普通 PCM 系统/独占输出、同 DAC PCM/DSD 双向快速交接、DSF/DFF/SACD 播放/暂停/定位/切曲、删除当前曲与裁剪/重排后继续播、容器激活/呈现、设备拔出/重接/忙碌/切换、关窗/退出释放、菜单与媒体键、PCM 独立文件同采样率连播与 CUE 首次播放听感。未测硬件不外推。
- 未验证范围 / 风险 / 阻塞：一次性打开 resolve 探针仍在调用 actor（有资源作用域），作为已盘点边界保留；无未记录阻塞。
- 验收是否通过及证据：通过。三仓库测试、公共协议 smoke、完整 `xcodebuild test`、`./run`、残留标识审查与全部手动回归（用户复验）均通过。
- 收尾结论：扩展边界重构收尾阶段 0–6 全部完成并通过最终验收。
...
- [x] 手工复验：同 DAC 关窗/切会话/重开与 PCM↔DSD 交接；混合 CUE/独立文件列表续播；快速 seek/暂停和界面状态。失败释放路径另由故障注入验证，不能仅靠听音认定通过。
