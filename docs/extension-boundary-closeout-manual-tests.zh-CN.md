...
日期：2026-09-11  
状态：阶段 3–6 所列手动项已全部由用户实机复验通过（2026-09-11）。  
...
## 阶段 4：菜单、动作状态与 P0 删除（2026-09-11 用户复验通过）
...
2. **设备菜单仍可切换**
   ...
   - 预期：列出连接的兼容 DAC，当前项勾选；断开或与当前 DSD 速率不兼容的设备显示为**禁用**（不可点击）；选择另一台兼容设备后声音与 Audio MIDI Setup 输出都切换。
   - 若点到不可用设备：不应发生切换，且**原设备继续播放、不暂停**；不得出现“切换失败但停在暂停”的状态。
3. **媒体键仍生效**
...
## 阶段 5：扫描、通用容器与余项（2026-09-11 用户复验通过）
